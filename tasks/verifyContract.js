...
function getContractInheritance(
  ...
  for (const [k, v] of parentContracts) {
    resp = {
      ...getContractInheritance(v["content"], remainingContractsNew, finalObj),
      ...resp,
    };
  }
...
function formatPutObj(
  ...
  for (const [license, type] of Object.entries(licenseTypes)) {
    if (
      baseContract[1]["content"].includes(`SPDX-License-Identifier: ${license}`)
    ) {
  ...
  if (contractBuildInfo["input"]["settings"]["optimizer"]["enabled"]) {
    ...
    putObj["runs"] =
      contractBuildInfo["input"]["settings"]["optimizer"]["runs"];
  } else {
